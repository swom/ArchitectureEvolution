...
namespace rndutils {
  ...
  public:
    ...
    {
      uint32_t tmp[4];
      sseq.generate(tmp, tmp + 4);
      std::memcpy(state_.data(), tmp, 2 * sizeof(uint64_t));
    }
  ...
  public:
    ...
    void seed(std::seed_seq& sseq)
    {
      pivot_ = 0;
      uint32_t tmp[32];
      sseq.generate(tmp, tmp + 32);
      std::memcpy(state_.data(), tmp, 16 * sizeof(uint64_t));
    }
